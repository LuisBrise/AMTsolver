...
if (config.dump_dldw) {
  ...
  out.open(filename);
  out.precision(17);
  out << "vv\t\t\tb\t\t\tw(au)\t\t\tdlEdwyInt\t\t\tdlHdwyInt\t\t\tdlEdwyScat\t\t\tdlHdwyScat\t\t\tdlEdwyExt\t\t\tdlHdwyExt\n";
}
...
void AMTsolver(SimulationConfig& config){
    ...
    std::cerr << "[AMTsolver] dump_dldw = " << std::boolalpha << config.dump_dldw << "\n";
